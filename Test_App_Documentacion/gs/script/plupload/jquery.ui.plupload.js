/* Customizations for Gallery Server: Look for [Roger] */
/**
	...
	*/
	...
	_handleState: function() {
		...
		if (up.total.queued === 0) {
		    this.browse_button.html(_('Add Files')); // [Roger] 2016-10-13 Tweaked for jQuery UI 1.12.1. Was: $('.ui-button-text', this.browse_button).html(_('Add Files'));
		} else {
		    this.browse_button.html(plupload.sprintf(_('%d files queued'), up.total.queued)); // [Roger] 2016-10-13 Tweaked for jQuery UI 1.12.1. Was: $('.ui-button-text', this.browse_button).html(plupload.sprintf(_('%d files queued'), up.total.queued));
		}
	...
	_enableViewSwitcher: function() {
		...
		if (buttons.length === 1) {
		    ...
		    // [Roger] 2016-10-07 Comment out "e.preventDefault()" to restore list/thmb showing the active selection. This causes scrolling to the list/thmb switcher but is a non-issue in GS because it is always visible anyway (i.e. near the top of the page). 
		    switcher
				.show()
				.buttonset()
				.find('.ui-button')
					...
					}).find('.ui-state-hover').removeClass('ui-state-hover'); // [Roger] 2016-10-13 Remove 'ui-state-hover' from buttons. This will no longer be necessary in jQuery UI 1.12.2 because that version will not add the class.
