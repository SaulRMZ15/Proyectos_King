    ...
    <v-layout align-start>
        <v-flex>
            <v-toolbar color="white">
                <v-btn color="primary" @click="crearPDF()"><v-icon color="white">mdi-printer</v-icon></v-btn>
                <v-divider class="mx-2" inset vertical></v-divider>
                <v-toolbar-title class="purple--text">Articulos</v-toolbar-title>
                <!-- <v-divider
                class="mx-2"
                inset
                vertical
                ...
                <v-dialog v-model="dialog" max-width="500px">
                    <template v-slot:activator="{ on }">
                        <v-btn color="primary" dark class="mb-2" v-on="on"><v-icon>mdi-view-grid-plus-outline</v-icon></v-btn>
                    </template>
                    <v-card>
                        <v-card-title>
                        ...
                        <v-container grid-list-md>
                            <v-layout wrap>
                                <v-flex xs12 sm6 md6>
                                    <v-text-field v-model="codigo" label="Código" color="#8933C0"></v-text-field>
                                </v-flex>
                                <v-flex xs12 sm6 md6>
                                ...
                                <v-flex xs12 sm6 md6>
                                    <v-text-field type="number" v-model="stock" label="Stock" color="#8933C0"></v-text-field>
                                </v-flex>
                                <v-flex xs12 sm6 md6>
                                    <v-text-field v-model="precio_venta" label="Precio" color="#8933C0"></v-text-field>
                                </v-flex>
                                <v-flex xs12 sm12 md12>
                                    <v-text-field v-model="descripcion" label="Descripción" color="#8933C0"></v-text-field>
                                </v-flex>
            ...
            <v-data-table :headers="headers" :items="articulos" :search="search"  class="elevation-1">
                ...
                <template v-slot:no-data>
                    <v-btn class="purple--text" color="white" @click="listar()">Resetear</v-btn>
                </template>
            </v-data-table>
        </v-flex>
    </v-layout>
    ...
    export default {
        data(){
            return{
                ...
                headers: [
                    ...
                    { text: 'Precio', value: 'precio_venta', sortable: false },
        ...
        methods: {
            crearPDF(){
                ...
                doc.save('Articulos_' + fecha.toLocaleDateString() +'.pdf')
            },
            selectCategoria(){
                let me=this;
                let categoriaArray=[];
            ...
            validar(){
                ...
                if(this.descripcion.length>255){
                    this.validaMensaje.push('La descripción del articulo no debe tener más de 255 caracteres.');
                }
                if(this.stock<0){
                    this.validaMensaje.push('Ingrese un stock valido');
                }
                if (this.precio_venta<=0) {
                    this.validaMensaje.push('Ingrese un valor de venta valido')
                }
